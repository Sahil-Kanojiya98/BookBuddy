...
public class ReviewService {
    ...
    public void deleteReview(UserPrincipal userPrincipal, Long bookId) {
        ...
        } else throw new RatingNotFoundException(String.format("Rating not found. bookId: %d", bookId));
    }

    @Transactional(readOnly = true)
    public Page<MyReviewResponse> searchMyReviews(UserPrincipal userPrincipal, ReviewSearchRequest reviewSearchRequest) {
        Long userId = userPrincipal.getId();

        Pageable pageable = buildPageable(reviewSearchRequest);

        return reviewRepository.searchMyReviews(
                userId,
    ...
    private Pageable buildPageable(ReviewSearchRequest reviewSearchRequest) {
        Sort sort = Sort.by(Sort.Order.by(reviewSearchRequest.getSortBy()).with(Sort.Direction.fromString(reviewSearchRequest.getSortOrder())));
        return PageRequest.of(reviewSearchRequest.getPage(), reviewSearchRequest.getSize(), sort);
    }
}
